...
class TopKHeap:
    ...
    def insert(self, elt):
        ...
        # print(size)  #- len(A)+len(H)=8+5=13
        # print(self.k) #5  len(A) =5
        ...
        if size <= self.k:
            ...
            self.H.insert(elt)
            print(self.H)
            #as A is asc sorted array already like min heap so A last element should be < H first element which is breaking in this case as 0 <-1
            if self.A[self.k-1] > self.H.min_element():
                self.insert_into_A(self.H.min_element())

        # if self.H.min_element() < self.A[self.k-1]:
        #     self.A[self.k - 1] ,  self.H[1] = self.H[1] , self.A[self.k - 1]
